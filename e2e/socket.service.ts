...
import { share, first, debounceTime, tap } from 'rxjs/operators';
...
export class SocketService {
    private host: string = 'ws://localhost:3001';
    ...
    emit<T = any, S = any>(event: string, data?: T, ack?: boolean): void | Observable<S> {
        if (ack) {
